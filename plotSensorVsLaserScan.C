/* 
   ...
   Compare ESQ plates location, as measured from the AMD laser scan and the VCNL4010 sensors.
   
   Usage:
   .x plotSensorVsLaserScan.C(<VC>)
...
int plotSensorVsLaserScan(Int_t VC=1 // vacuum chamber id
			  ){
  ...
  if (VC==1){
    ...
    theta_low=300.9;
    theta_high=313.86;
    theta_survey[0]=360-49.735; theta_survey[1]=360-49.920; theta_survey[2]=0; theta_survey[3]=0;
    r_survey[0]=7061.043; r_survey[1]=7061.100; r_survey[2]=0; r_survey[3]=0;
    run_low=4053;
    run_high=4124;
    run_low_US=3931;
    run_high_US=4006;
  ...
  printf("LaserScan inner <r> = %5.2f +- %4.2f\n" , hin_l->GetMean(), hin_l->GetMeanError());
  printf("Scan-Sensors diff inner <r> = %5.2f\n" , (hin_l->GetMean()-hin_s->GetMean()));
  ...
  printf("LaserScan outer <r> = %5.2f +- %4.2f\n" , hout_l->GetMean(), hout_l->GetMeanError());
  printf("Scan-Sensors diff outer <r> = %5.2f\n" , (hout_l->GetMean()-hout_s->GetMean()));
  ...
  for (int i_range=0; i_range<n_ranges; i_range++){
    ...
    if (VC<6) entries=tree_in->Draw("1000*r >> h_range_in",
				    Form("abs(z)<0.01 && -1*theta>%f && -1*theta<%f", theta_range_low, theta_range_high));
    else entries=tree_in->Draw("1000*r >> h_range_in",
			       Form("abs(z)<0.01 && 360-theta>%f && 360-theta<%f", theta_range_low, theta_range_high));
    if (entries==0) {
      printf("Empty range #%d, skipping.\n", i_range);
      n_empty_ranges++; continue;
    }
    TH1* h_range_in = (TH1*)gPad->GetPrimitive("h_range_in");
    if (VC<6) entries=tree_out->Draw("1000*r >> h_range_out",
				     Form("abs(z)<0.01 && -1*theta>%f && -1*theta<%f", theta_range_low, theta_range_high));
    else entries=tree_out->Draw("1000*r >> h_range_out",
				Form("abs(z)<0.01 && 360-theta>%f && 360-theta<%f", theta_range_low, theta_range_high));
    if (entries==0) {
      printf("Empty range #%d, skipping.\n", i_range);
      n_empty_ranges++; continue;
    }
    TH1* h_range_out = (TH1*)gPad->GetPrimitive("h_range_out");
    r_range_in[i_range-n_empty_ranges] = h_range_in->GetMean();
    err_r_range_in[i_range-n_empty_ranges] = h_range_in->GetMeanError();
    r_range_out[i_range-n_empty_ranges] = h_range_out->GetMean();
    err_r_range_out[i_range-n_empty_ranges] = h_range_out->GetMeanError();
    h_range_in->Delete(); h_range_out->Delete();
    //printf("<theta>=%f , <r_in>=%f , <r_out>=%f \n", theta_range_mean[i_range-n_empty_ranges], r_range_in[i_range-n_empty_ranges], r_range_out[i_range-n_empty_ranges]);
    r_range_diff[i_range-n_empty_ranges] = r_range_out[i_range-n_empty_ranges]-r_range_in[i_range-n_empty_ranges];
    err_r_range_diff[i_range-n_empty_ranges] = TMath::Sqrt(TMath::Power(err_r_range_in[i_range-n_empty_ranges],2)+
							   TMath::Power(err_r_range_out[i_range-n_empty_ranges],2));
  }
  ...
  TGraphErrors *gr_range = new TGraphErrors(n_ranges-n_empty_ranges, theta_range_mean, r_range_diff, 0, err_r_range_diff);
  gr_range->SetMarkerStyle(20); gr_range->SetMarkerSize(0.8); 
  mg_sum->Add(gr_range);
  mg_sum->Draw("AP");
  mg_sum->SetTitle(Form("Distance between plates, VC%d; #theta (degrees); d (mm)", VC));
  ...
  gr_range->Fit("pol0","Q");
  TF1* fit_range=gr_range->GetFunction("pol0");
  fit_range->SetLineStyle(2); fit_range->SetLineColor(kBlack);
  printf("LaserScan <Dr> = %5.2f +- %4.2f\n" , fit_range->GetParameter(0), fit_range->GetParError(0));
  printf("Scan-Sensors diff <Dr> = %5.2f\n" , fit_range->GetParameter(0)-fit_sensor_sum->GetParameter(0));
  ...
  TMultiGraph *mg;
  c1 = new TCanvas("c_comparison","c_comparison",100,100,2000,1200);
  c1->Divide(2,1);
  c1->cd(1);
  ...
  mg->Draw("AP");
  mg->SetTitle(Form("Inner plate radial location, VC%d; #theta (degrees); r (mm)", VC));
  ...
  mg->Add(gr_laser_middle);
  mg->Draw("AP");
  mg->SetTitle(Form("Outer plate radial location, VC%d; #theta (degrees); r (mm)", VC));
